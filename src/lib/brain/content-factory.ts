...
import { bandingkanGelombang, skorTrend, type TrendGelombang, type TrendItem } from "./trend-radar";
...
export function rencanaKonten(brain: BrainMemory, trends: TrendItem[], gelombang: TrendGelombang[] | null, n = 7): SlotKonten[] {
  ...
  const trendNaikCocok = (gelombang || [])
    .filter((g) => (g.status === "naik" || g.status === "baru") && skorTrend(g.title).cocokLagu)
    .map((g) => ({ topik: g.title, src: "trend-naik" as const, alasan: `${g.status === "naik" ? "🌊 naik" : "🆕 baru"} & cocok niche` }));
  const trendCocok = (trends || [])
    .filter((t) => skorTrend(t.title).cocokLagu)
    .map((t) => ({ topik: t.title, src: "trend-cocok" as const, alasan: "cocok niche (cerita jadi lagu)" }));
